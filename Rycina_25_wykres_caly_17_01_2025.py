"""
Wykres wielu serii z Excela (tylko wartości), z osią czasu podaną w kodzie.
Dodane pionowe linie zdarzeń:
- X=66  : Rozpoczęcie hipoksji
- X=91  : Zakończenie hipoksji
"""

import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
from matplotlib.ticker import MultipleLocator, FuncFormatter

# ==== KONFIGURACJA ====
EXCEL_PATH = "wykres_caly_2024_12_20_2.xlsx"  # <- ścieżka do Excela (tylko wartości)
SHEET_NAME = 0                                # nazwa arkusza lub indeks (0)
HAS_HEADER = False                            # brak nagłówków kolumn -> False

FIGSIZE = (11, 7.5)
FONT_FAMILY = "Times New Roman"
AXIS_FONTSIZE = 20
TICK_FONTSIZE = 18
LINEWIDTH = 1.5
MARKERSIZE = 3
XTICK_STEP_MIN = 20
SAVE_PNG = False
PNG_PATH = "wykres_czas_komorki.png"
DPI = 300

# ==== USTAWIENIA MATPLOTLIB ====
plt.rcParams["font.family"] = FONT_FAMILY

# ==== OŚ CZASU (minuty) – Twoja lista punktów czasowych ====
TIME_POINTS = np.array([
    0, 5, 10, 15, 20, 25, 30, 35, 40, 45,
    50, 55, 60, 66, 71, 76, 81, 86, 91, 99,
    104, 109, 114, 119, 124, 129, 134, 139,
    144, 149, 154, 159, 164, 169, 174, 179,
    184, 188, 193, 198, 203, 208, 213, 218
], dtype=float)

# ==== WCZYTANIE DANYCH Z EXCELA (tylko wartości) ====
header_arg = 0 if HAS_HEADER else None
df = pd.read_excel(EXCEL_PATH, sheet_name=SHEET_NAME, header=header_arg)

# Zamiana na liczby i usunięcie pustych wierszy
df = df.apply(pd.to_numeric, errors="coerce").dropna(how="all")

# Dopasowanie długości danych do liczby punktów czasu
n_time = len(TIME_POINTS)
n_rows = len(df)
if n_rows < n_time:
    TIME_POINTS = TIME_POINTS[:n_rows]
else:
    df = df.iloc[:n_time, :]

# Interpolacja braków
df = df.interpolate(method="linear", axis=0, limit_direction="both")

# ==== RYSOWANIE ====
fig, ax = plt.subplots(figsize=FIGSIZE)

# Serie pomiarowe (bez legendy)
for col in df.columns:
    ax.plot(TIME_POINTS, df[col].values, "-o", linewidth=LINEWIDTH, markersize=MARKERSIZE)

# --- Pionowe linie zdarzeń ---
line_reo = ax.axvline(
    x=66, color="black", linestyle=":", linewidth=4, zorder=10,
    label="Rozpoczęcie hipoksji"
)
line_hyp = ax.axvline(
    x=91, color="black", linestyle="--", linewidth=3, zorder=10,
    label="Zakończenie hipoksji"
)

# Opisy osi
ax.set_xlabel("Czas (min)", fontsize=AXIS_FONTSIZE, labelpad=15)
ax.set_ylabel("Znormalizowany stosunek fluorescencji czerony/zielony", fontsize=AXIS_FONTSIZE, labelpad=15)
ax.tick_params(axis="both", labelsize=TICK_FONTSIZE)

# Zakresy osi
ax.set_xlim(-2, float(TIME_POINTS.max()) + 2)
ax.xaxis.set_major_locator(MultipleLocator(XTICK_STEP_MIN))

# --- Formatowanie przecinki zamiast kropek ---
ax.yaxis.set_major_formatter(FuncFormatter(lambda y, _: f"{y:.1f}".replace(".", ",")))

# Siatka
ax.grid(True, linestyle=":", linewidth=0.8)

# Legenda tylko dla zdarzeń
ax.legend(
    handles=[line_reo, line_hyp],
    loc="lower right",
    facecolor="white",
    framealpha=1,
    edgecolor="grey",
    fontsize=16
)

plt.tight_layout()

if SAVE_PNG:
    plt.savefig(PNG_PATH, dpi=DPI, bbox_inches="tight")

plt.show()
